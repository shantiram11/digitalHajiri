 ...
 */
final class DeviceMappingController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        return response()->json([
            'data' => EmployeeDeviceMapping::query()->latest('id')->paginate($request->integer('per_page', 25)),
        ]);
    }

    public function show(int $id): JsonResponse
    {
        return response()->json(['data' => EmployeeDeviceMapping::query()->findOrFail($id)]);
    }
}
